...
function Flight() {
	...
	useEffect(() => {
		fetch('http://localhost:4000/flight').then((res) =>
			res.json()
		).then((result) =>
			setData(result)
			// console.log(result)
					...
					<div class="col py-3" style={{ backgroundColor: '#e0f4ff' }} >
						...
						<div style={{ backgroundColor: '#f8f8f8', padding: '10px' }}>
							<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
								<h2>Table</h2>
								<Link to='/flightadd' style={{ background: 'green', color: 'white', padding: '10px', textDecoration: 'none',borderRadius: '5px' }}>Flight</Link>
							</div>
							<br/>
							<div>
								<table border="1" className="table table-striped">
									<thead>
										<tr>
											...
											<th style={{width:'25%'}}>Flight Name</th>
											<th style={{width:'25%'}}>Number of Seats</th>
									...
									<tbody>
										{data.map((item,index) => (
											<tr key={index}>
												...
												<td>{item.Seat}</td>
								                <td><Link to="/flightedit" state={{id:item._id}} style={{background: '#007bff',color: 'white',padding: '7px',textDecoration: 'none',borderRadius: '5px',display: 'inline-block',}}>Edit</Link></td>
